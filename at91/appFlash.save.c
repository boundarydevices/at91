      ...
      case INIT:
      {
         if( sizeof(msgFlashRequest_t) <= fragLength ){
            ...
            switch( msg->msgType_ ){
               ...
               {
                  if(msg->length_ <= FLASH_PAGE_SIZE){
...
NODEBUGMSG( "prog 0x" ); NODEBUGHEX(msg->offs_); NODEBUGMSG(": 0x"); NODEBUGHEX(rval); NODEBUGMSG("\r\n");
                     }
                     else {
			NODEBUGMSG( "rx0 " ); NODEBUGHEX( bytesReceived ); NODEBUGMSG( " of " ); NODEBUGHEX( bytesExpected ); NODEBUGMSG( "\r\n" );
                     ...
                     NODEBUGMSG( "flashPage too long 0x" ); NODEBUGHEX(msg->length_); NODEBUGMSG("\r\n");
                     response_.rval_ = -3 ;
                     state = RESPONSE ;
                     usbll_transmit_app( USBEP_BULKIN, USBAPP_FLASH, sg, flashUsbResponseComplete, 0 );
                  }
      ...
      case MOREDATA:
      {
         ...
         NODEBUGMSG( "rx1 " ); NODEBUGHEX( bytesReceived ); NODEBUGMSG( " of " ); NODEBUGHEX( bytesExpected );  NODEBUGMSG( "\r\n" );
         if( bytesReceived == bytesExpected ){
            response_.rval_ = flashProg( FLASH_BASE+flashOffs_, bytesReceived, flashData_ );
NODEBUGMSG( "writeFlash offs " ); NODEBUGHEX( flashOffs_ ); NODEBUGMSG( ", rval " ); NODEBUGHEXCHAR( response_.rval_ ); NODEBUGMSG( "\r\n" );
            state = RESPONSE ;
            usbll_transmit_app( USBEP_BULKIN, USBAPP_FLASH, sg, flashUsbResponseComplete, 0 );
         }
         break ;
